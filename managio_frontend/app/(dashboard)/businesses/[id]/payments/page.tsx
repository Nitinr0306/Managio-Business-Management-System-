  ...
  const columns = useMemo<ColumnDef<Payment>[]>(() => [
    {
      ...
      cell: ({ row }) => (
        <div>
          ...
          {row.original.publicId && <div className="text-[10px] text-indigo-300/80 font-medium">{row.original.publicId}</div>}
                  ...
                  <div key={String(p.id)} className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        {p.publicId && <div className="text-[10px] text-indigo-300/80 font-medium mt-0.5 truncate">{p.publicId}</div>}
