...
export default function StaffPage() {
  ...
  const { data, isLoading, refetch } = useStaff(businessId, {
    page,
    size: 20,
        ...
        cell: ({ getValue }) => (
          <span
            className={cn(
              ...
              ROLE_COLORS[getValue() as string] || ROLE_COLORS.STAFF
            )}
          >
            {getValue() as string}
          </span>
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <span
            className={cn(
              'text-xs px-2.5 py-1 rounded-full font-medium',
              STATUS_COLORS[getValue() as string] || STATUS_COLORS.INACTIVE
            )}
          >
            {getValue() as string}
          </span>
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Link
              href={`/businesses/${businessId}/staff/invitations`}
              className="flex items-center gap-2 px-4 py-2.5 border border-white/10 text-white/70 hover:text-white hover:bg-white/5 text-sm font-medium rounded-xl transition-all"
            >
