  ...
  const onMarkPaid = (staffId: string, salaryMonth: string, pendingAmount: number) => {
    markPaid.mutate({
      ...
      payload: {
        ...
        notes: 'Marked paid from salary ledger',
      },
    })
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Track monthly salary payment status, pending amounts, and mark payouts."
        icon={Wallet}
        breadcrumbs={[
          { label: 'Businesses', href: '/businesses' },
          { label: 'Staff', href: `/businesses/${businessId}/staff` },
          { label: 'Salary Ledger' },
        ]}
        actions={
          <div className="flex items-center gap-2">
            <input
              ...
              className="h-10 px-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-white/75 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
            />
            <Link
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="rounded-2xl border border-white/[0.06] bg-[hsl(var(--card))] p-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <section className="xl:col-span-2 rounded-2xl border border-white/[0.06] bg-[hsl(var(--card))] overflow-hidden">
          ...
          {monthlyLoading ? (
            ...
            <div className="p-6">
              <EmptyState
                ...
                description="Salary ledger entries are created once staff salary is configured and active."
              />
            </div>
          ) : (
                ...
                <tbody>
                  {monthly.map((row) => (
                    <tr key={row.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3">
                        {row.paymentStatus === 'UNPAID' && row.pendingAmount > 0 ? (
                          <button
                          ...
                          <span className="text-xs text-white/35">{row.paidAt ? formatDateTime(row.paidAt) : 'Already Paid'}</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        <section className="rounded-2xl border border-white/[0.06] bg-[hsl(var(--card))] overflow-hidden">
          ...
          {unpaidLoading ? (
            ...
            <div className="p-6">
              <EmptyState
                ...
                description="All salary records are currently settled for this month."
              />
            </div>
          ) : (
