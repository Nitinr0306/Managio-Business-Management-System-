...
export default function SettingsPage() {
  ...
  const [notifications, setNotifications] = useState(() => {
    ...
    const saved = localStorage.getItem('managio-notification-prefs')
  ...
  const [privacySettings, setPrivacySettings] = useState(() => {
    if (typeof window === 'undefined') return { usageAnalytics: true, errorReports: true }
    const saved = localStorage.getItem('managio-privacy-prefs')
    return saved ? JSON.parse(saved) : { usageAnalytics: true, errorReports: true }
  })

  const updateNotification = (key: string) => {
    setNotifications((p: typeof notifications) => {
      const updated = { ...p, [key]: !p[key as keyof typeof p] }
      localStorage.setItem('managio-notification-prefs', JSON.stringify(updated))
      toast.success('Notification preference updated')
      return updated
    })
  }

  const updatePrivacy = (key: keyof typeof privacySettings) => {
    setPrivacySettings((p: typeof privacySettings) => {
      const updated = { ...p, [key]: !p[key] }
      localStorage.setItem('managio-privacy-prefs', JSON.stringify(updated))
  ...
  const inp =
    'w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white placeholder-white/25 focus:outline-none focus:border-indigo-500/60 transition-all'
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* Tab pills */}
      <div className="flex items-center gap-1 flex-wrap bg-white/[0.03] border border-white/[0.06] rounded-2xl p-1.5 mb-6">
        {TABS.map((tab) => (
          <button
            key={tab.key}
      ...
      {activeTab === 'privacy' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item.key} className="flex items-center justify-between py-1">
                  ...
                  <Toggle checked={privacySettings[item.key]} onChange={() => updatePrivacy(item.key)} />
                </div>
              ))}
            </div>
          </SectionCard>
          ...
          <div className="p-5 rounded-2xl border border-red-500/15 bg-red-500/5">
            ...
            <button
              onClick={() => toast.error('Account deletion requires contacting support at support@managio.com. This action is irreversible.', { duration: 8000 })}
              className="flex items-center gap-2 px-4 py-2 rounded-xl border border-red-500/25 text-red-400/80 text-xs hover:bg-red-500/10 hover:text-red-400 transition-all">
              <AlertTriangle className="w-3.5 h-3.5" />
              Delete My Account
            </button>
          </div>
