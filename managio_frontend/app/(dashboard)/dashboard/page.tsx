...
import { formatCurrency, formatDate, formatRelative, getDaysRemaining } from '@/lib/utils/formatters'
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
      ...
      >
        <div className="relative mx-auto w-24 h-24 mb-6">
          ...
          <div className="relative w-24 h-24 rounded-3xl bg-indigo-500/15 border border-indigo-500/20 flex items-center justify-center">
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl transition-all shadow-xl shadow-indigo-600/30 hover:-translate-y-0.5"
          >
          ...
          <Link
            href="/businesses/new"
            className="inline-flex items-center justify-center gap-2 px-6 py-3 border border-white/10 text-white/60 hover:text-white hover:bg-white/5 rounded-xl transition-all text-sm"
          >
  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-5 mb-5">
        {/* Revenue area chart */}
        <div className="lg:col-span-2 p-5 rounded-2xl border border-white/6 bg-white/[0.02]">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-display font-600 text-white/80">Revenue Trend</h3>
            <span className="text-xs text-white/30 bg-white/4 px-2.5 py-1 rounded-lg">Last 12 months</span>
          </div>
          {false ? (
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={[]} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ background: '#0f0f1a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, fontSize: 12 }}
            ...
            <div className="h-[200px] flex flex-col items-center justify-center text-white/25">
              ...
              <div className="space-y-2 mt-3">
                {paymentMethods.slice(0, 4).map((m: any, i: number) => (
                  <div key={m.method} className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-medium text-white/70">{m.percentage?.toFixed(0)}%</span>
                  </div>
                ))}
              </div>
            </>
          ...
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-display font-600 text-white/80 flex items-center gap-2">
              ...
              {expiring && expiring.length > 0 && (
                <span className="text-xs px-1.5 py-0.5 rounded-md bg-amber-500/15 text-amber-400">{expiring.length}</span>
              )}
            </h3>
              ...
              {expiring.slice(0, 6).map((sub) => (
                <div key={`${sub.memberId}-${sub.endDate}`} className="flex items-center justify-between py-2.5 px-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] transition-all group">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-white/35 truncate mt-0.5">{sub.planName}</div>
                  </div>
                  <div className={cn(
              ...
              {recentPayments.slice(0, 6).map((p) => (
                <div key={p.paymentId} className="flex items-center justify-between py-2.5 px-3 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] transition-all">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-white/35 mt-0.5">{formatRelative(p.paidAt)} · {p.paymentMethod}</div>
                  </div>
...
export default function DashboardPage() {
  ...
  // Redirect staff/member to their portals
  useEffect(() => {
