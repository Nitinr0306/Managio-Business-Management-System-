  ...
  if (!canManageSubscriptions) {
    return (
      <div className="max-w-2xl">
        <PageHeader title="Subscriptions" description="You don't have access to manage subscriptions." icon={CreditCard} />
        <div className="p-6 rounded-2xl border border-white/6 bg-white/[0.02] text-sm text-white/50">
          Ask the owner to grant you subscription-management permissions.
        </div>
      </div>
    )
  }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link
              href="/staff/subscriptions/assign"
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {data?.content.map(sub => {
                        const daysLeft = getDaysRemaining(sub.endDate)
                        return (
                          <tr key={String(sub.id)} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2.5">
                                <div className="w-7 h-7 rounded-full bg-emerald-600/20 flex items-center justify-center text-xs font-bold text-emerald-300">
                                  {(sub.memberName || '?').slice(0, 2).toUpperCase()}
                                </div>
                                <div>
                                  <div className="text-sm font-medium text-white/80">{sub.memberName || '—'}</div>
                                  {sub.memberEmail && <div className="text-xs text-white/35">{sub.memberEmail}</div>}
                                </div>
                              </div>
                            </td>
                            <td className="px-4 py-3">
