...
import {
  ...
  BarChart3, ScrollText, ChevronLeft, ChevronRight, Dumbbell,
  LogOut, Settings, ChevronDown, Plus, Calendar, Bell, HelpCircle, Mail,
} from 'lucide-react'
...
export function Sidebar({ collapsed, onToggle, mobileOpen = false }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed left-0 top-0 h-screen bg-[#080812] border-r border-white/5 flex flex-col z-40 transition-[width] duration-300 ease-out overflow-hidden',
          wide ? 'w-[260px]' : 'w-[72px]',
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 h-16 border-b border-white/5 flex-shrink-0">
          <Link href="/dashboard" className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-indigo-500/25">
            <Dumbbell className="w-4 h-4 text-white" />
          </Link>
          {wide && (
            <motion.span
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="text-base font-display font-700 whitespace-nowrap flex-1"
            >
              Managio
            </motion.span>
          )}
        ...
        {wide && (
          <div className="px-3 py-2.5 border-b border-white/5 flex-shrink-0">
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-white/4 hover:bg-white/6 transition-all"
            >
              <div className="w-7 h-7 rounded-lg bg-indigo-600/30 flex items-center justify-center text-xs font-display font-700 text-indigo-300 flex-shrink-0">
              ...
              <ChevronDown className={cn('w-3.5 h-3.5 text-white/40 transition-transform flex-shrink-0', bizOpen && 'rotate-180')} />
            </button>
            ...
            <AnimatePresence>
              {bizOpen && (
                ...
                >
                  <div className="mt-1 space-y-0.5 max-h-44 overflow-y-auto scrollbar-thin">
                    <button
                      onClick={() => { setCurrentBusiness(null); setBizOpen(false); router.push('/dashboard') }}
                      className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/4 transition-all text-left"
                    >
                      <div className="w-6 h-6 rounded-md bg-white/5 flex items-center justify-center text-[10px] text-white/40">A</div>
                      <span className="text-xs text-white/60">All Businesses</span>
                    </button>
                    {(businesses || []).map((biz) => (
                      <button
                        key={biz.id}
                        ...
                        className={cn(
                          'w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/4 transition-all text-left',
                          currentBusiness?.id === biz.id && 'bg-indigo-500/10'
                        )}
                      >
                        <div className="w-6 h-6 rounded-md bg-indigo-600/20 flex items-center justify-center text-[10px] font-700 text-indigo-300">
                          {getInitials(biz.name)}
                        </div>
                        <span className="text-xs text-white/70 truncate">{biz.name}</span>
                      </button>
                    ))}
                    <Link
                      href="/businesses/new"
                      ...
                      className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/4 transition-all text-left"
                    >
                      <div className="w-6 h-6 rounded-md border border-dashed border-white/15 flex items-center justify-center">
                        <Plus className="w-3 h-3 text-white/40" />
                      </div>
                      <span className="text-xs text-white/40">Add business</span>
                    </Link>
                  </div>
                </motion.div>
              )}
        ...
        <nav className="flex-1 overflow-y-auto scrollbar-thin py-3 px-2.5">
          {navGroups.map((group, gi) => (
            <div key={gi} className="mb-4">
              {group.label && wide && (
                <div className="px-2 py-1 text-[10px] font-600 text-white/25 uppercase tracking-wider mb-1">
                  {group.label}
                </div>
              )}
        ...
        <div className="border-t border-white/5 p-3 flex-shrink-0">
          {wide ? (
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500/30 to-violet-500/30 border border-indigo-500/20 flex items-center justify-center text-xs font-display font-700 text-indigo-300 flex-shrink-0">
              ...
              <button onClick={() => logout()} className="text-white/25 hover:text-red-400 transition-colors" title="Sign out">
            ...
            <button
              onClick={() => logout()}
              className="w-full flex items-center justify-center py-2 rounded-xl text-white/25 hover:text-red-400 hover:bg-red-500/10 transition-all"
              title="Sign out"
            >
              <LogOut className="w-4 h-4" />
            </button>
