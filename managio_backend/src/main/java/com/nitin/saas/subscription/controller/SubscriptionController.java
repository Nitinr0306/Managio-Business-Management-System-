...
public class SubscriptionController {
    ...
    @GetMapping
    @Operation(summary = "List subscriptions for a business (paginated)")
    public ResponseEntity<Page<MemberSubscriptionResponse>> getBusinessSubscriptions(
            @PathVariable Long businessId,
            Pageable pageable) {
        return ResponseEntity.ok(subscriptionService.getBusinessSubscriptions(businessId, pageable));
    }

    @PostMapping("/{subscriptionId}/cancel")
    @Operation(summary = "Cancel a member subscription")
    public ResponseEntity<Void> cancelSubscription(
            @PathVariable Long businessId,
            @PathVariable Long subscriptionId,
            @RequestParam(required = false, defaultValue = "Cancelled by staff") String reason) {
        subscriptionService.cancelSubscription(businessId, subscriptionId, reason);
        return ResponseEntity.ok().build();
    }
