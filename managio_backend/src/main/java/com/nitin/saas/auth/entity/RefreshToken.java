...
@Table(name = "refresh_tokens", indexes = {
        ...
        @Index(name = "idx_refresh_subject", columnList = "userId, subjectType")})
...
public class RefreshToken {
    ...
    /** USER (owner/staff tokens) or MEMBER (member-portal tokens). */
    @Column(nullable = false, length = 20)
    @Builder.Default private String subjectType = "USER";
    ...
    @Column(length = 500) private String replacedByToken;
    ...
    @Column(nullable = false) @Builder.Default private Integer rotationCount = 0;
    private LocalDateTime lastRotatedAt;
    @Version private Long version;

    public boolean isExpired() { return LocalDateTime.now().isAfter(expiresAt); }
    public boolean isValid()   { return !revoked && !used && !isExpired(); }

    public void revoke() { revoked = true; revokedAt = LocalDateTime.now(); }
    public void markAsUsed() { used = true; usedAt = LocalDateTime.now(); }
    public void rotate(String newToken) {
        replacedByToken = newToken; rotationCount++; lastRotatedAt = LocalDateTime.now();
        revoke();
    }
}